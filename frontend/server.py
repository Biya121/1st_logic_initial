...
각 시나리오마다:
  - reason 필드는 반드시 최소 4문장 이상의 상세한 한 문단으로 작성하세요. 포함해야 할 내용: ①이 가격대를 선택한 근거(참조가 대비 몇 % 조정인지), ②이 시장에서의 포지셔닝 전략(경쟁사 대비 어떤 포지션인지), ③이 가격이 적합한 상황 또는 타깃 바이어 유형, ④예상되는 리스크 또는 주의사항. 짧은 한 줄 요약은 절대 금지합니다.
  - 구체적인 계산식을 formula 필드에 작성하세요 (예: {ref_hint} × 0.90 = SGD {float(ref_price)*0.90:.2f} 형식).
rationale은 최소 4문장으로 싱가포르 시장 거시 환경·제품 경쟁력·판정 근거·진입 시 주요 리스크를 포함해 상세히 서술하세요.
...
참조가가 미확인이라면 시장 데이터·경쟁사·제품 특성을 기반으로 합리적인 가격을 추정하세요."""
        ...
        def _fb_sc(base: float, market_ko: str) -> list:
            agg  = round(base * 0.88, 2)
            cons = round(base * 1.12, 2)
            return [
                {"name": "저가 진입", "price_sgd": agg,
                 "reason": (
                     f"참조가 SGD {base:.2f} 대비 12% 할인한 SGD {agg:.2f}로 책정하여 {market_ko} 초기 진입 단계에서 가격 경쟁력을 극대화합니다. "
                     f"싱가포르 {market_ko}은 신규 공급사에 대한 검증 기간이 필요하므로, 낮은 진입가로 조달 담당자 및 바이어의 관심을 끌어 첫 계약을 확보하는 데 유리합니다. "
                     f"일단 공급 실적이 쌓이면 이후 협상에서 가격 정상화가 가능하므로, 초기 저마진을 감수하더라도 레퍼런스 확보를 우선시하는 전략입니다. "
                     f"단, 지나치게 낮은 가격은 제품 품질에 대한 신뢰도를 떨어뜨릴 수 있으므로, 현지 에이전트와 함께 가격 방어 논리를 준비해야 합니다."
                 ),
                 "formula": f"참조가 SGD {base:.2f} × 0.88 = SGD {agg:.2f}"},
                {"name": "기준가", "price_sgd": base,
                 "reason": (
                     f"참조가 SGD {base:.2f}를 기준가로 설정하여 {market_ko}에서 리스크와 마진의 균형을 유지합니다. "
                     f"이 가격대는 싱가포르 시장 내 동등 성분 또는 유사 제품의 평균 가격 범위에 해당하며, 경쟁사 대비 동등 가치(value parity) 포지셔닝을 취합니다. "
                     f"안정적인 마진을 확보하면서도 가격 저항 없이 입찰·등재 절차를 진행할 수 있어, 지속 가능한 공급 관계 구축에 가장 적합합니다. "
                     f"환율 변동·물류비 인상 등 원가 리스크에 대응할 버퍼가 내재되어 있으므로, 장기 계약 조건에서도 수익성 방어가 가능합니다."
                 ),
                 "formula": f"참조가 SGD {base:.2f} (기준가 그대로)"},
                {"name": "프리미엄", "price_sgd": cons,
                 "reason": (
                     f"참조가 SGD {base:.2f} 대비 12% 프리미엄을 적용한 SGD {cons:.2f}로 책정하여 {market_ko}에서 고부가가치 포지셔닝을 추구합니다. "
                     f"싱가포르 민간 병원·전문 클리닉 채널에서는 브랜드 인지도와 규제 이력이 우수한 제품에 대해 프리미엄 수용도가 높으며, 글로벌 브랜드와 동등한 수준의 가격 책정이 가능합니다. "
                     f"이 전략은 고마진 확보와 함께 브랜드 포지셔닝 강화를 동시에 달성할 수 있으며, 리베이트·번들 조건 등 협상 여지도 충분히 남겨둘 수 있습니다. "
                     f"단, 기존 시장 지배적 브랜드와의 직접 경쟁 구도에서는 임상 데이터·품질 차별화 근거를 명확히 제시해야 가격 정당성을 확보할 수 있습니다."
                 ),
                 "formula": f"참조가 SGD {base:.2f} × 1.12 = SGD {cons:.2f}"},
            ]

        analysis: dict[str, Any] = {}
        try:
            raw_analysis = analysis_resp.content[0].text
            # 마크다운 코드블록 제거 후 직접 파싱 시도
            raw_stripped = re.sub(r"^```(?:json)?\s*", "", raw_analysis.strip())
            raw_stripped = re.sub(r"\s*```$", "", raw_stripped.strip())
            try:
                analysis = json.loads(raw_stripped)
            except Exception:
                m_json2 = re.search(r"\{.*\}", raw_stripped, re.S)
                if m_json2:
                    analysis = json.loads(m_json2.group(0))
        except Exception:
            pass

        # 파싱 실패 또는 필수 키 누락 시 폴백 — ref_price를 기준가로 사용
        if not analysis.get("public_market") or not analysis.get("private_market"):
            est_pub  = round(ref_price if ref_price else 10.0, 2)
            est_priv = round((ref_price * 1.2) if ref_price else 12.0, 2)
            analysis = {
                "rationale": (
                    f"참조가 SGD {ref_price:.2f} 기준 기본값으로 산정합니다."
                    if ref_price else "참조가 미확인 — 기본값으로 산정합니다."
                ),
        ...
        # AI 시나리오 필드명 정규화 (PDF generator는 label/price/price_usd 사용)
        _sgd_usd_rate = exchange_rates.get("sgd_usd", 0)

        def _norm_sc(raw: list) -> list:
            result = []
            for sc in (raw or []):
                price_sgd = sc.get("price_sgd", sc.get("price", 0)) or 0
                price_usd = round(price_sgd / _sgd_usd_rate, 2) if _sgd_usd_rate > 0 else 0
                result.append({
        ...
        def _to_usd(sgd_val: float) -> float:
            return round(sgd_val / _sgd_usd_rate, 2) if _sgd_usd_rate > 0 else 0

        pub_data  = analysis.get("public_market",  {}) or {}
        priv_data = analysis.get("private_market", {}) or {}

        pub_base  = pub_data.get("final_price_sgd",  0) or 0
        priv_base = priv_data.get("final_price_sgd", 0) or 0

        sections = [
            {
                "seg_label":      "공공 시장 (ALPS·조달청 채널)",
                "base_price":     pub_base,
                "base_price_usd": _to_usd(pub_base),
                "scenarios":      _norm_sc(pub_data.get("scenarios", [])),
            },
            {
                "seg_label":      "민간 시장 (병원·약국·체인 채널)",
                "base_price":     priv_base,
                "base_price_usd": _to_usd(priv_base),
...
class P2ScenarioItem(BaseModel):
    label: str
    price: float        # SGD
    price_usd: float = 0  # USD (카드 표시값과 동일)
    reason: str = ""
    formula: str = ""
...
class P2SectionItem(BaseModel):
    seg_label: str
    base_price: float          # SGD
    base_price_usd: float = 0  # USD
...
async def p2_regenerate_pdf(body: P2RegenBody) -> JSONResponse:
    ...
    sections = [
        {
            ...
            "base_price_usd": sec.base_price_usd,
            "scenarios": [
                {
                    "label":     sc.label,
                    "price":     sc.price,
    ...
    pub_base_usd = body.sections[0].base_price_usd if body.sections else 0
    ...
    p2_data = {
        ...
        "seg_label":       "공공·민간 시장 통합",
        "base_price":      pub_base,
        "base_price_usd":  pub_base_usd,
        "sgd_usd":         body.sgd_usd,
        "formula_str":     "",
        "mode_label":      body.mode_label,
        "macro_text":      body.macro_text,
